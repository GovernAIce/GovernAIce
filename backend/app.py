...
def get_domains():
    ...
    Updated to include relevant policy domains.
    """
    domains = [
        "Autonomous Vehicles",
...
# --- USE CASE 2 ---
def analyze_policy_opportunities(project_description, countries, domains, doc_id, model="gemini"):
    """
    Analyze project description to identify policy opportunities and government incentives.
    Uses the specified LLM model to discover relevant policies, subsidies, and application strategies.
    """
    if not project_description or not project_description.strip():
        logger.error("Empty or invalid project description provided for policy opportunity analysis")
        return {
            ...
            "opportunity_score": 0,
            "summary": "Unable to analyze: Empty project description"
        }

    # Create comprehensive prompt for policy opportunity discovery
    prompt = f"""
    You are an expert policy analyst specializing in government incentives, subsidies, and business development opportunities. 
    Analyze the following project description to identify relevant policy opportunities across the specified countries and domains.

    Project Description: {project_description}
    Target Countries: {', '.join(countries)}
    Domains: {', '.join(domains) if domains else 'All domains'}
    ...
    {{
        "government_incentives": {{
            "subsidies": [
                {{
                    "name": "string - name of subsidy program",
                    "description": "string - brief description",
                    "country": "string - country offering the subsidy",
                    "eligibility": "string - eligibility criteria",
                    "amount": "string - funding amount or range"
                }}
            ],
            "investments": [
                {{
                    "name": "string - investment program name",
                    "description": "string - brief description",
                    "country": "string - country offering investment",
                    "focus_area": "string - area of focus",
                    "amount": "string - investment amount or range"
                }}
            ],
            "national_strategies": [
                {{
                    "name": "string - strategy name",
                    "description": "string - how project aligns with national strategy",
                    "country": "string - country with this strategy",
                    "relevance": "string - relevance to project"
                }}
            ]
        }},
        "application_support": {{
            ...
            "funding_sources": [
                "string - funding source 1",
                "string - funding source 2",
                "string - funding source 3"
            ],
            "timeline": "string - recommended application timeline"
        }},
        "opportunity_score": integer between 0 and 100,
        "summary": "string - brief summary of opportunities identified"
    }}

    Focus on realistic, actionable opportunities that align with the project description and target countries.
    """
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f"Attempt {attempt + 1} for policy opportunity analysis")
                response_text = analyze_policy(prompt, model=model)
                if response_text:
        ...
        opportunity_data = extract_json_from_response(response_text)
        
        if opportunity_data is None:
            logger.error(f"Failed to extract JSON from response: {response_text}")
            return create_fallback_opportunity_response("Invalid JSON response format")

        # Validate required fields
        required_fields = ['government_incentives', 'application_support', 'opportunity_score', 'summary']
        ...
        if missing_fields:
            ...
            return create_fallback_opportunity_response(f"Missing required fields: {missing_fields}")

        # Validate opportunity score
        if not isinstance(opportunity_data['opportunity_score'], int) or not (0 <= opportunity_data['opportunity_score'] <= 100):
            logger.warning(f"Invalid opportunity_score: {opportunity_data.get('opportunity_score')}")
            opportunity_data['opportunity_score'] = 0

        return opportunity_data

    except Exception as e:
        logger.error(f"Policy opportunity analysis failed: {str(e)}")
        return create_fallback_opportunity_response(str(e))

def create_fallback_opportunity_response(error_msg):
    """Create a fallback response when policy opportunity analysis fails."""
    return {
        "government_incentives": {
            "subsidies": [],
            "investments": [],
        ...
        "application_support": {
            "strategies": [f"Analysis failed: {error_msg}"],
            "funding_sources": [],
            "timeline": "Unable to determine"
        },
        "opportunity_score": 0,
        "summary": f"Policy opportunity analysis failed: {error_msg}"
    }

# Add these new endpoints to your Flask app

@app.route('/policy-opportunities/upload/', methods=['POST'])
def upload_policy_opportunity_analysis():
    """
    Upload a project document and analyze it for policy opportunities.
    Expects multipart form-data with 'file', 'countries' (JSON list), and optional 'domains' (JSON list).
    Returns: doc_id, filename, and policy opportunity analysis.
    """
    if 'file' not in request.files or 'countries' not in request.form:
        return jsonify({'error': 'Missing file or countries'}), 400
    ...
    try:
        ...
        if not isinstance(countries, list) or not isinstance(domains, list):
            return jsonify({'error': 'Countries and domains must be lists'}), 400
            
    except json.JSONDecodeError:
        return jsonify({'error': 'Invalid countries or domains format'}), 400
    ...
    project_description = extract_text_from_pdf(file)
    doc_id = str(uuid.uuid4())
    filename = secure_filename(file.filename)
    ...
    try:
        ...
        document = {
            ...
            'countries': countries,
            'domains': domains,
            'analysis_type': 'policy_opportunity',
            'created_at': datetime.utcnow(),
            'policy_opportunities': {}
        }
        mongo_db.policy_opportunities.insert_one(document)

        # Analyze for policy opportunities
        opportunities = analyze_policy_opportunities(project_description, countries, domains, doc_id)

        # Update document with analysis results
        mongo_db.policy_opportunities.update_one(
            {'doc_id': doc_id},
            {'$set': {
        ...
        logger.error(f"Error in policy opportunity upload: {str(e)}")
        return jsonify({'error': str(e)}), 500
    finally:
        mongo_client.close()

@app.route('/policy-opportunities/analyze/', methods=['POST'])
def analyze_policy_opportunity_text():
    """
    Analyze project description text for policy opportunities.
    Expects JSON body with 'project_description', 'countries' (list), and optional 'domains' (list).
    Returns: doc_id, analysis results with government incentives and application support.
    """
    try:
        data = request.get_json()
        if not data or 'project_description' not in data or 'countries' not in data:
            return jsonify({'error': 'Missing project_description or countries'}), 400
        ...
        if len(project_description) < 10:
            return jsonify({'error': 'Project description must be at least 10 characters'}), 400
        ...
        doc_id = str(uuid.uuid4())
        doc_title = f"Policy_Opportunity_{doc_id[:8]}"
        ...
        try:
            ...
            logger.error(f"Error in policy opportunity text analysis: {str(e)}")
            return jsonify({'error': str(e)}), 500
        finally:
            mongo_client.close()
...
@app.route('/policy-opportunities/', methods=['GET'])
def list_policy_opportunities():
    """
    List all policy opportunity analyses.
    Returns: Array of policy opportunity analysis objects.
    """
    mongo_client = get_mongo_client()
    try:
        mongo_db = mongo_client['regulatory_mongo']
        opportunities = mongo_db.policy_opportunities.find().sort('created_at', -1)
        ...
        logger.error(f"Error fetching policy opportunities: {str(e)}")
        return jsonify({'error': str(e)}), 500
    finally:
        mongo_client.close()

@app.route('/policy-opportunities/<doc_id>/', methods=['GET'])
def get_policy_opportunity_detail(doc_id):
    """
    Get details for a specific policy opportunity analysis by doc_id.
    Returns: Policy opportunity analysis object or 404 if not found.
    """
    mongo_client = get_mongo_client()
    try:
        mongo_db = mongo_client['regulatory_mongo']
        opportunity = mongo_db.policy_opportunities.find_one({'doc_id': doc_id})
        
        if not opportunity:
            return jsonify({'error': 'Policy opportunity analysis not found'}), 404
        ...
        logger.error(f"Error fetching policy opportunity {doc_id}: {str(e)}")
        return jsonify({'error': str(e)}), 500
    finally:
        mongo_client.close()

@app.route('/policy-opportunities/search/', methods=['GET'])
def search_policy_opportunities():
    """
    Search policy opportunities by project description, country, or domain.
    Query parameters: query, country, domain
    Returns: Array of matching policy opportunity objects.
    """
    query = request.args.get('query', '')
    ...
    try:
        ...
        if country:
            search_filter['countries'] = {'$in': [country]}
            
        if domain:
            search_filter['domains'] = {'$in': [domain]}
            
        opportunities = mongo_db.policy_opportunities.find(search_filter).sort('created_at', -1)
        ...
        logger.error(f"Error searching policy opportunities: {str(e)}")
        return jsonify({'error': str(e)}), 500
    finally:
        mongo_client.close()

# --- USE CASE 4: REGULATORY DOCUMENT GENERATION ---

def generate_compliance_report(project_description, countries, domains, report_id, model="gemini"):
    """
    Generate a standardized compliance report based on project description, countries, and domains.
    Uses the specified LLM model to create a report that meets local regulatory requirements.
    Returns a JSON object with report content and metadata.
    """
    if not project_description or not project_description.strip():
        logger.error("Empty or invalid project description provided for compliance report generation")
        return {
            "report_id": report_id,
            "content": {"summary": "Empty project description", "sections": []},
            "metadata": {"countries": countries, "domains": domains, "generated_at": datetime.utcnow().isoformat()},
    ...
    valid_countries = set(get_country_policies().keys())
    if not all(c in valid_countries for c in countries):
        logger.error("Invalid country specified in compliance report generation")
        return {
            "report_id": report_id,
            "content": {"summary": "Invalid country specified", "sections": []},
            "metadata": {"countries": countries, "domains": domains, "generated_at": datetime.utcnow().isoformat()},
            "compliance_status": "failed",
            "error": "Invalid country specified"
        }

    # Create prompt for compliance report generation
    prompt = f"""
    You are an expert in generating regulatory compliance reports for multiple jurisdictions and industries.
    Based on the following project description, generate a standardized compliance report that meets the regulatory requirements of the specified countries and domains.
    ...
    IMPORTANT: Return ONLY valid JSON in the exact format below, with no additional text, comments, or formatting:
    {{
        "summary": "string - brief summary of the compliance report",
        "sections": [
            {{
                "country": "string - country name",
                "domain": "string - industry/domain name",
                "requirements": ["string - requirement 1", "string - requirement 2"],
                "compliance_status": "string - compliant, non-compliant, or partially compliant",
                "recommendations": ["string - recommendation 1", "string - recommendation 2"]
            }}
        ]
    }}

    Ensure the report is concise, actionable, and tailored to the regulatory requirements of each country and domain.
    ...
    try:
        ...
        for attempt in range(max_retries):
            try:
                logger.debug(f"Attempt {attempt + 1} for compliance report generation")
                response_text = analyze_policy(prompt, model=model)
                if response_text:
                    logger.debug(f"Raw response: {response_text[:300]}...")
                    break
        ...
        # Extract and validate JSON response
        report_content = extract_json_from_response(response_text)
        if report_content is None:
            logger.error(f"Failed to extract JSON from response: {response_text}")
            return {
                "report_id": report_id,
                "content": {"summary": "Invalid JSON response format", "sections": []},
                "metadata": {"countries": countries, "domains": domains, "generated_at": datetime.utcnow().isoformat()},
                "compliance_status": "failed",
        ...
        if missing_fields:
            ...
            return {
                "report_id": report_id,
                "content": {"summary": f"Missing required fields: {missing_fields}", "sections": []},
                "metadata": {"countries": countries, "domains": domains, "generated_at": datetime.utcnow().isoformat()},
                "compliance_status": "failed",
        ...
        for section in report_content["sections"]:
            section_required_fields = ["country", "domain", "requirements", "compliance_status", "recommendations"]
            section_missing = [field for field in section_required_fields if field not in section]
            if section_missing:
                logger.warning(f"Invalid section in response: missing {section_missing}")
                return {
                    "report_id": report_id,
                    "content": {"summary": f"Invalid section data: missing {section_missing}", "sections": []},
                    "metadata": {"countries": countries, "domains": domains, "generated_at": datetime.utcnow().isoformat()},
                    "compliance_status": "failed",
                    "error": f"Invalid section data: missing {section_missing}"
                }
        ...
        return {
            "report_id": report_id,
            "content": {"summary": f"Report generation failed: {str(e)}", "sections": []},
            "metadata": {"countries": countries, "domains": domains, "generated_at": datetime.utcnow().isoformat()},
            "compliance_status": "failed",
            "error": str(e)
        }

# Endpoint to generate compliance report from uploaded file
@app.route('/regulatory-reports/generate/upload/', methods=['POST'])
def generate_compliance_report_upload():
    """
    Generate a compliance report from an uploaded project document.
    Expects multipart form-data with 'file', 'countries' (JSON list), and optional 'domains' (JSON list).
    Returns: report_id, filename, and compliance report content.
    """
    if 'file' not in request.files or 'countries' not in request.form:
        return jsonify({'error': 'Missing file or countries'}), 400
    ...
    try:
        ...
        report = generate_compliance_report(project_description, countries, domains, report_id)
        ...
        mongo_db.regulatory_reports.insert_one(document)
        ...
        logger.error(f"Error in compliance report upload: {str(e)}")
        return jsonify({'error': str(e)}), 500
    finally:
        mongo_client.close()

# Endpoint to generate compliance report from text input
@app.route('/regulatory-reports/generate/text/', methods=['POST'])
def generate_compliance_report_text():
    """
    Generate a compliance report from project description text.
    Expects JSON body with 'project_description', 'countries' (list), and optional 'domains' (list).
    Returns: report_id, filename, and compliance report content.
    """
    try:
        ...
        report_id = str(uuid.uuid4())
        filename = f"Compliance_Report_{report_id[:8]}"
        ...
        try:
            ...
            logger.error(f"Error in compliance report text generation: {str(e)}")
            return jsonify({'error': str(e)}), 500
        finally:
            mongo_client.close()
...
# Endpoint to list all compliance reports
@app.route('/regulatory-reports/', methods=['GET'])
def list_compliance_reports():
    """
    List all generated compliance reports.
    Returns: Array of compliance report objects.
    """
    mongo_client = get_mongo_client()
    try:
        mongo_db = mongo_client['regulatory_mongo']
        reports = mongo_db.regulatory_reports.find().sort('created_at', -1)
...
# Endpoint to retrieve a specific compliance report
@app.route('/regulatory-reports/<report_id>/', methods=['GET'])
def get_compliance_report_detail(report_id):
    """
    Get details for a specific compliance report by report_id.
    Returns: Compliance report object or 404 if not found.
    """
    mongo_client = get_mongo_client()
    try:
        mongo_db = mongo_client['regulatory_mongo']
        report = mongo_db.regulatory_reports.find_one({'report_id': report_id})
        ...
        logger.error(f"Error fetching compliance report {report_id}: {str(e)}")
        return jsonify({'error': str(e)}), 500
    finally:
        mongo_client.close()

# Endpoint to update a compliance report
@app.route('/regulatory-reports/<report_id>/', methods=['PUT'])
def update_compliance_report(report_id):
    """
    Update an existing compliance report with new project description, countries, or domains.
    Expects JSON body with optional 'project_description', 'countries', and 'domains'.
    Returns: Updated compliance report object.
    """
    try:
        data = request.get_json()
        if not data:
        ...
        try:
            ...
            if project_description:
                report_data = generate_compliance_report(project_description, countries or report['metadata']['countries'], domains or report['metadata']['domains'], report_id)
                update_fields['content'] = report_data['content']
                update_fields['compliance_status'] = report_data['compliance_status']
                update_fields['error'] = report_data['error']
            if countries:
                update_fields['metadata.countries'] = countries
            if domains:
                update_fields['metadata.domains'] = domains
            if update_fields:
                update_fields['updated_at'] = datetime.utcnow()

            # Update report in MongoDB
            mongo_db.regulatory_reports.update_one(
                {'report_id': report_id},
                {'$set': update_fields}
            )

            # Fetch updated report
            updated_report = mongo_db.regulatory_reports.find_one({'report_id': report_id})
            updated_report['_id'] = str(updated_report['_id'])
            return jsonify(updated_report), 200

        except Exception as e:
            logger.error(f"Error updating compliance report {report_id}: {str(e)}")
            return jsonify({'error': str(e)}), 500
        finally:
            mongo_client.close()
...
# Endpoint to search compliance reports
@app.route('/regulatory-reports/search/', methods=['GET'])
def search_compliance_reports():
    """
    Search compliance reports by project description, country, or domain.
    Query parameters: query, country, domain
    Returns: Array of matching compliance report objects.
    """
    query = request.args.get('query', '')
    country = request.args.get('country', '')
    domain = request.args.get('domain', '')
    ...
    try:
        ...
        if country:
            search_filter['metadata.countries'] = {'$in': [country]}
        if domain:
            search_filter['metadata.domains'] = {'$in': [domain]}

        reports = mongo_db.regulatory_reports.find(search_filter).sort('created_at', -1)
        ...
        logger.error(f"Error searching compliance reports: {str(e)}")
        return jsonify({'error': str(e)}), 500
    finally:
        mongo_client.close()
