...
const EURiskLevelFrameworkWidget: React.FC = () => {
  const [riskBarData, setRiskBarData] = useState<EURiskBarData[]>([]);
  ...
  useEffect(() => {
      ...
      .then((data) => {
        if (data && data.data) {
          setRiskBarData(data.data);
        }
      })
      .catch((err) => console.error('Error fetching EU risk level data:', err));
  }, []);

  const totalScore = riskBarData.length
    ? Math.round(
        riskBarData.reduce(
          (sum, d) => sum + d.riskScore + d.mitigationScore,
          0
        ) / maxPossibleScore
      )
    : 0;
    ...
    return (
      <Card className="custom-border relative p-2 h-full">
        <div className="flex flex-col items-center justify-center h-full text-center">
          ...
          <h3 className="text-xl text-[#1975d4] font-bold mb-2">
            EU Risk Level Framework
          </h3>
          <p className="text-gray-600 text-sm">
            Upload a document and select a country to view EU risk level
            analysis
          </p>
        </div>
      </Card>
    ...
    <Card className="custom-border flex flex-row items-center p-4 h-full">
      <div className="flex-1 flex flex-col justify-center min-w-[220px]">
        <h2 style={{ fontSize: '12', color: '#1975D4', fontWeight: 700 }}>
          ...
          <br />
          Framework{' '}
          <span className="ml-2" style={{ color: '#1975D4', fontWeight: 700 }}>
            {totalScore}/8
          </span>
        </h2>
        <p className="text-base mt-2" style={{ color: '#000000' }}>
          Compare use case to the EU AI Risk and give overall scores about both
          of the risk and mitigation levels.
        </p>
      ...
      <div className="relative flex-1 flex flex-col justify-center items-center h-full">
        <svg
          viewBox={`0 0 ${chartWidth + leftMargin + 20} ${
            riskBarData.length * yStep + 60
          }`}
          className="w-full"
          style={{ height: `${riskBarData.length * yStep + 60}px` }}
        >
          ...
          {riskBarData.map((d, i) => {
            ...
            return (
              ...
              >
                {main}
                {rest.length > 0 && <tspan x={0} dy={14}>{rest.join('-')}</tspan>}
              </text>
            );
          })}
